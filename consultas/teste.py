...
def estimativa():
    ...
    faturamento = merged_data.groupby('grupo_do_produto').apply(lambda x: (x['quantidade_do_produto_vendida'])).sum().reset_index(name='total_vendido')
    fig, ax = plt.subplots()
    sns.pieplot(data=faturamento, x='total_vendido', y='grupo_do_produto', ax=ax)
    return fig
